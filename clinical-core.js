...
function getRangeWHO(z, type) {
    ...
    if (type === 'bbu') {
        if (z < -3) return `z = ${zs} < -3 (Severely Underweight)`;
        if (z < -2) return `-3 ≤ z = ${zs} < -2 (Underweight)`;
        if (z < -1) return `-2 ≤ z = ${zs} < -1 (Normoweight - Bawah Median)`;
        if (z <= 1) return `-1 ≤ z = ${zs} ≤ +1 (Normoweight - Ideal/Median)`;
        if (z <= 2) return `+1 < z = ${zs} ≤ +2 (Risk Overweight)`;
        return `z = ${zs} > +2 (Overweight)`;
    }
    if (type === 'tbu') {
        if (z < -3) return `z = ${zs} < -3 (Severely Stunted)`;
        if (z < -2) return `-3 ≤ z = ${zs} < -2 (Stunted)`;
        if (z < -1) return `-2 ≤ z = ${zs} < -1 (Normoheight - Cenderung Pendek)`;
        if (z <= 1) return `-1 ≤ z = ${zs} ≤ +1 (Normoheight - Ideal/Median)`;
        if (z <= 3) return `+1 < z = ${zs} ≤ +3 (Normoheight - Cenderung Tinggi)`;
        return `z = ${zs} > +3 (Tall Stature)`;
    }
    if (type === 'bbtb') {
        if (z < -3) return `z = ${zs} < -3 (Gizi Buruk)`;
        if (z < -2) return `-3 ≤ z = ${zs} < -2 (Gizi Kurang)`;
        if (z < -1) return `-2 ≤ z = ${zs} < -1 (Gizi Baik - Cenderung Kurang)`;
        if (z <= 1) return `-1 ≤ z = ${zs} ≤ +1 (Gizi Baik - Ideal/Median)`;
        if (z <= 2) return `+1 < z = ${zs} ≤ +2 (Gizi Baik - Cenderung Lebih)`;
        if (z <= 3) return `+2 < z = ${zs} ≤ +3 (Gizi Lebih)`;
        return `z = ${zs} > +3 (Obesitas)`;
    }
    if (type === 'imtu') {
        if (z < -3) return `z = ${zs} < -3 (Severe Thinness)`;
        if (z < -2) return `-3 ≤ z = ${zs} < -2 (Thinness)`;
        if (z < -1) return `-2 ≤ z = ${zs} < -1 (Normal - Bawah Median)`;
        if (z <= 1) return `-1 ≤ z = ${zs} ≤ +1 (Normal - Ideal/Median)`;
        if (z <= 2) return `+1 < z = ${zs} ≤ +2 (Overweight Risk)`;
        if (z <= 3) return `+2 < z = ${zs} ≤ +3 (Overweight)`;
        return `z = ${zs} > +3 (Obese)`;
    }
    return '';
}
...
function getRangeCDC(pct, type) {
    ...
    if (type === 'bmi') {
        if (pct < 5) return `${p} < P5 (Underweight)`;
        if (pct < 25) return `P5 ≤ ${p} < P25 (Healthy Weight - Cenderung Rendah)`;
        if (pct < 75) return `P25 ≤ ${p} ≤ P75 (Healthy Weight - Ideal/Median)`;
        if (pct < 85) return `P75 < ${p} < P85 (Healthy Weight - Cenderung Tinggi)`;
        if (pct < 95) return `P85 ≤ ${p} < P95 (Overweight)`;
        return `${p} ≥ P95 (Obesity)`;
    }
    
    if (type === 'stature') {
        if (pct < 3) return `${p} < P3 (Short Stature)`;
        if (pct < 10) return `P3 ≤ ${p} < P10 (Normostature - Cenderung Pendek)`;
        if (pct < 25) return `P10 ≤ ${p} < P25 (Normostature - Bawah Median)`;
        if (pct < 75) return `P25 ≤ ${p} ≤ P75 (Normostature - Ideal/Median)`;
        if (pct <= 90) return `P75 < ${p} ≤ P90 (Normostature - Atas Median)`;
        if (pct <= 97) return `P90 < ${p} ≤ P97 (Normostature - Tinggi)`;
        return `${p} > P97 (Tall Stature)`;
    }
    
    // weight (BB/U)
    if (pct < 3) return `${p} < P3 (Underweight)`;
    if (pct < 10) return `P3 ≤ ${p} < P10 (Normoweight - Cenderung Kurang)`;
    if (pct < 25) return `P10 ≤ ${p} < P25 (Normoweight - Bawah Median)`;
    if (pct < 75) return `P25 ≤ ${p} ≤ P75 (Normoweight - Ideal/Median)`;
    if (pct <= 90) return `P75 < ${p} ≤ P90 (Normoweight - Atas Median)`;
    if (pct <= 97) return `P90 < ${p} ≤ P97 (Overweight)`;
    return `${p} > P97 (Obesitas)`;
}
        ...
        if (isWHO) {
            ...
            for (let i = 0; i < bands.length - 1; i++) {
                if (z >= bands[i] && z < bands[i + 1]) {
                    return `antara garis ${fmt(bands[i])} dan ${fmt(bands[i + 1])}`;
                }
            }
            if (z < -3) return 'di bawah garis -3 SD';
            return 'di atas garis +3 SD';
        } else {
            const p = Number(pct);
            const bands = [3, 5, 10, 25, 50, 75, 85, 90, 95, 97];
            for (let i = 0; i < bands.length - 1; i++) {
                if (p >= bands[i] && p < bands[i + 1]) {
                    return `antara garis P${bands[i]} dan P${bands[i + 1]}`;
                }
            }
            if (p < 3) return 'di bawah garis P3';
            return 'di atas garis P97';
        }
    }
